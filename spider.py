...
while counter < 100:
    ...
    if (counter == 0):
        this_url = f"https://www.dcard.tw/_api/forums/nutc/posts?popular=false&limit=100"
    ...
    for post in list_of_post:
        ...
        if post.content:
            post = re.sub(r'^jsonp\d+\(|\)\s+$', '', post.text)
            ...
            # 塞入陣列
            p = [post_id, post_title, post_content, post_createdAt, post_updatedAt, post_commentCount, post_likeCount, post_topics]
        ...
        if comment.content:
            ...
            for com_json in comment_json:
                if (com_json['hiddenByAuthor'] == False | com_json['hidden'] == False):
                    ...
                    # 塞入陣列
                    c = [comment_post_id, comment_floor, comm_content, comment_createdAt, comment_updatedAt, comment_likeCount]
...
df=pd.DataFrame(allpost,columns = ["id","title","content","createdAt","updatedAt","commentCount","likeCount","topics"]) 
df.head()
...
post_filename = os.path.join(cwd,'dcard_nutc_post_{}.csv'.format(timestamp))
df.to_csv(post_filename,index = False, encoding='utf_8_sig')
print('Save csv to {}'.format(post_filename))

comment_filename = os.path.join(cwd,'dcard_nutc_comment_{}.csv'.format(timestamp))
